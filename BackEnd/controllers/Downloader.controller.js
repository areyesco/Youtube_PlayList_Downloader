const downloaderScript = require('../Scripts/Donwloader.script')
...
class DownloaderController{
    ...
    //obtain the data of the playlist
    async showPlaylistSongs(req,res){
        res.status(200)
        .send(await downloaderScript.getPlaylistData(req.body.playlistID))
    }
    ...
    async storeAndDonwloadSongs(req,res){
        let PLAYLIST_ID = req.body.id;
        let Items = await this.getPlaylistSongs();
        Items(PLAYLIST_ID).then( async(data) =>{
            ...
            await downloadVideos(PLAYLIST_ID)
        })

    }

    async  downloadVideos(PLAYLIST_ID){
        try {
    ...
    //obtains the number of videos that have the value false in the attribute downloaded
     getVideosToDownloadByPlaylist(videos, idPlaylist){
        let counter = 0;
